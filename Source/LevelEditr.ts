/// <reference path="GroupHoldr/GroupHoldr.ts" />
/// <reference path="InputWritr/InputWritr.ts" />
/// <reference path="MapsHandlr/MapsHandlr.ts" />
/// <reference path="PixelDrawr/PixelDrawr.ts" />
/// <reference path="StatsHoldr/StatsHoldr.ts" />
/// <reference path="TimeHandlr/TimeHandlr.ts" />
...
class LevelEditr {
    ...
    constructor(settings: ILevelEditrSettings) {
        ...
        this.prethings = settings.things;
    ...
    */
    ...
    private setCurrentArgs(): void {
        if (this.currentClickMode === "Thing") {
            this.setCurrentThing(this.currentTitle, this.getCurrentArgs());
        } else {
            this.onMacroIconClick(this.currentTitle, undefined, this.getCurrentArgs());
        }
    }
    ...
    */
    ...
    private setMapName(): void {
        ...
        if (map && map.name !== name) {
            ...
            this.GameStarter.StatsHolder.set("world", name);
        }
    }
    ...
    private setMapTime(fromGui: boolean): void {
        ...
        this.setTextareaValue(this.stringifySmart(map), true);
        this.GameStarter.StatsHolder.set("time", time);
    ...
    private setLocationArea(): void {
        var map: IMapsCreatrMapRaw = this.getMapObject(),
            location: IMapsCreatrLocationRaw;
     ...
     * 
     */
    private getCurrentLocationObject(map: IMapsCreatrMapRaw): IMapsCreatrLocationRaw {
            ...
            "children": (function (): HTMLElement[] {
                var selectedIndex: number = 0,
                    containers: HTMLElement[] = Object.keys(scope.prethings).map(function (key: string): HTMLDivElement {
                        var children: HTMLDivElement[] = Object.keys(scope.prethings[key]).map(function (title: string): HTMLDivElement {
    ...
    */
    ...
    private disableAllThings(): void {
        ...
        this.GameStarter.GroupHolder.getGroup("Character").forEach(function (thing: IThing): void {
            scope.disableThing(thing);
        });

        this.GameStarter.StatsHolder.set("time", Infinity);
    }
    ...
    private clearAllThings(): void {
        var groups: { [i: string]: IThing[] } = this.GameStarter.GroupHolder.getGroups(),
            i: string,
            j: number;
        ...
        for (i in groups) {
            if (groups.hasOwnProperty(i)) {
                for (j = 0; j < groups[i].length; j += 1) {
                    this.GameStarter.killNormal(groups[i][j]);
                }
            }
        }
    }
